...
public class MinimapHUD {
    ...
    private final int mapSize = 64; // ミニマップのサイズ(px)
    private final int zoomSize = 4;  // 1チャンクを何ピクセルで描くか
    int margin = 5;
    ...
    public void onRenderOverlay(RenderGameOverlayEvent.Post event) {
        ...
        // 1. 背景と枠（少しグレーの枠を付けると締まって見えます）
        // Gui.drawRect(-1, -1, mapSize + 1, mapSize + 1, 0xFFFFFFFF); // 白い外枠
        Gui.drawRect(0, 0, mapSize, mapSize, 0xFF000000);           // 黒い背景

        // 2. チャンク描画
        int pX = mc.player.chunkCoordX;
        int pZ = mc.player.chunkCoordZ;

        // 表示範囲の計算（mapSizeが64なら、8チャンク分くらい表示）
        int range = (mapSize / zoomSize) / 2;
        for (int x = -range; x <= range; x++) {
            for (int z = -range; z <= range; z++) {
                // 中心(mapSize/2)から、チャンク座標に応じてオフセット
        ...
        // 4. 座標表示（マップが小さいので、文字も少し小さくするか下に配置）
        String coords = String.format("%d, %d", (int) mc.player.posX, (int) mc.player.posZ);
        GlStateManager.scale(0.5, 0.5, 0.5); // 文字を50%の大きさに
        mc.fontRenderer.drawStringWithShadow(coords, (mapSize - (float) mc.fontRenderer.getStringWidth(coords) / 2),
    ...
    private void renderMinimapChunk(int rx, int rz, int dx, int dy) {
        ...
        if (colors != null) {
            ...
            AsyncMapRenderer.requestChunk(mc.player.world, rx, rz);
        }
        ...
        if (d != null) {
            ...
            // renderMinimapChunkの中の領地描画付近
            // dx, dy が 0 未満や mapSize を超える場合は描画しないガードを入れる
            if (dx >= 0 && dy >= 0 && dx < mapSize && dy < mapSize) {
    ...
    private void drawSmallPlayerIcon(int centerX, int centerY) {
        float rotation = mc.player.rotationYaw + 180.0F;
