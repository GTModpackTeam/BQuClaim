 ...
 * new wire ID and no churn in {@link ModNetwork} or
...
public class ClientNotify implements IMessage {
     ...
     * and string payload defined by {@link PartyAction}; the on-wire format lives there.
     */
    public static final class PartyActionDispatcher implements IMessageHandler<PartyAction, IMessage> {
        ...
        private static boolean disbandParty(ActionContext c) {
            ...
            c.pendingNotifications.add(() -> {
                for (UUID memberId : members) {
                    ...
                    if (member != null) {
                        notifyPlayer(member, EVENT_DISBANDED, "", "");
                    }
                }
            });
            return true;
        ...
        private static boolean invitePlayer(ActionContext c) {
            ...
            if (inviterParty != null && !inviterParty.canAddMember()) {
                notifyPlayer(c.player, EVENT_PARTY_FULL, "", "");
                return false;
            }
            ...
            if (srv != null) {
                ...
                if (target != null) {
                    ...
                    c.pendingNotifications.add(() -> notifyPlayer(target, EVENT_INVITE_RECEIVED,
        ...
        private static boolean acceptInvite(ActionContext c) {
            ...
            if (targetParty == null) {
                ...
                notifyPlayer(c.player, EVENT_JOIN_FAILED, "", "");
                return false;
            }
            ...
            if (joinedParty != null) {
                ...
                MinecraftServer srv = c.player.getServer();
                c.pendingNotifications.add(() -> notifyPartyMembers(joinedParty, EVENT_MEMBER_JOINED,
        ...
        private static boolean kickOrLeave(ActionContext c) {
            ...
            if (party != null) {
                String event = isSelf ? EVENT_MEMBER_LEFT : EVENT_KICKED;
        ...
        private static boolean changeRole(ActionContext c) {
            ...
            if (srv != null && target != null) {
                ...
                c.pendingNotifications.add(() -> notifyPlayer(target, EVENT_ROLE_CHANGED,
                        targetName, newRole));
            }
            return true;
        }
        ...
        private static boolean toggleBQuLink(ActionContext c) {
            ...
            if (party != null) {
                String event = linked ? EVENT_BQU_LINKED : EVENT_BQU_UNLINKED;
                MinecraftServer srv = c.player.getServer();
        ...
        private static boolean transferOwnership(ActionContext c) {
            ...
            c.pendingNotifications.add(() -> {
                notifyPlayer(target, EVENT_OWNER_TRANSFERRED, newOwnerName, "");
                notifyPlayer(sender, EVENT_ROLE_CHANGED, senderName, "ADMIN");
            });
            return true;
        }
        ...
        private static boolean joinFreeParty(ActionContext c) {
            ...
            String joinerName = c.player.getName();
            MinecraftServer srv = c.player.getServer();
            c.pendingNotifications.add(() -> notifyPartyMembers(party, EVENT_MEMBER_JOINED,
                    joinerName, "", srv, joinerId));
            return true;
        }
        ...
        private static void notifyPartyMembers(Party party, String eventType, String playerName, String extra,
            ...
            ClientNotify packet = partyEvent(eventType, playerName, extra);
        ...
        private static void notifyPlayer(EntityPlayerMP player, String eventType, String playerName, String extra) {
            ModNetwork.INSTANCE.sendTo(partyEvent(eventType, playerName, extra), player);
        }
